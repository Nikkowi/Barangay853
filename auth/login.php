...
if ($requires2FA) {
    ...
    // Build a short-lived pre-auth token
    ...
    $storeStmt = $conn->prepare(
        "INSERT INTO personal_access_tokens
         ...
         VALUES ('2fa_pending', ?, 'pre_auth', ?, '[\"*\"]', NOW(), ?)"
    );
    ...
    // ── Failsafe: check if a valid unused code already exists ─────────────────
    ...
    if ($existingCode) {
        // Reuse the existing code — don't send a new email, keep the original timer
        $expiresAt = $existingCode['expires_at'];
        $twoFaCode = $existingCode['code'];

        // Calculate remaining seconds for the frontend timer
        $remainingSecs = strtotime($expiresAt) - time();
        $expiryMins    = max(1, ceil($remainingSecs / 60));

    } else {
        // No valid code exists — generate and send a fresh one
    ...
    $logAction = '2FA Initiated';
    $logModule = 'System';
    $logRefId  = 'USER-' . $user['id'] . ' (' . $user['role'] . ')';
...
// ── No 2FA required (residents) — issue token immediately ────────────────────
